...
class conexao {
    ...
    private $bd = "cartonearte";  
    private $user = "root"; 
    private $password = ""; 
    private $conn = ""; 
    public function __construct(){      
    try{ $this->conn = new PDO("mysql:host={$this->server};
    dbname={$this->bd};
    charset=utf8;",
    $this->user,$this->password);          }
    catch(PDOException $msg){echo "Não foi possível fazer a conexão com o servidor: ".$msg->getMessage();
  ...
  public function executeQuery($comando){
    try{
        ...
        $sql->execute(); var_dump($sql->errorInfo()); die(); 
        if($sql->rowCount() > 0){
            return '1';
        }else{
